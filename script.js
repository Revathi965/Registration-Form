...
function validPhone(){
    ...
    // isNaN(phoneInput) == false 
       phoneInput.match(/^[0-9]+$/) 
    // phoneInput.match(/^[a-zA-Z]+$/) === null
       ){
...
// ============= Validation for CTC ==============
function validCTC(){
    let CTCInput = document.getElementById('CTC').value
    if(CTCInput && CTCInput.length <= 7 && CTCInput.match(/^[0-9]+$/)){  // If CTCInput is empty String it returns " Falsy " means it executes else condition...
        document.getElementById('CTC-valid').style.display = 'block'
        document.getElementById('CTC-invalid').style.display = 'none'
        // error = false;
    }
    else{
        document.getElementById('CTC-valid').style.display = 'none'
        document.getElementById('CTC-invalid').style.display = 'block'
        error = true;
    }
}
